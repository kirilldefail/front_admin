...
type SortField = 'name' | 'version' | 'category' | 'size' | null;
...
const mockApplications: Application[] = [
  {
    ...
    category: 'Разработка',
    size: '85',
    osVersions: ['Debian 11+', 'Debian 12+'],
    status: 'active',
  ...
  },
  {
    ...
    category: 'Браузеры',
    size: '65',
    osVersions: ['Debian 10+', 'Debian 11+'],
    status: 'active',
...
export function ApplicationsList({ onAddNew, onEdit }: ApplicationsListProps) {
  ...
  const filteredAndSortedApplications = useMemo(() => {
    let filtered = mockApplications.filter(app => {
      ...
      const matchesOsVersion = osVersionFilter === 'all' || app.osVersions.includes(osVersionFilter);
  ...
  const osVersions = Array.from(new Set(mockApplications.flatMap(app => app.osVersions)));
